...
import { isLogged }        from './utils/session.js';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function validaCampos() {
    ...
    if (!cpfInput.value || !validateCPF(cpfInput.value)) {
      document.getElementById('cpfGroup').classList.add('error'); ok = false;
    }
    if (!passwordInput.value) {
      document.getElementById('passwordGroup').classList.add('error'); ok = false;
    }
    return ok;
  }
});
